...
class kcp_accounttransfer extends kcp_card_gateway
{
    public function get_the_id() {
        $pay_ids = gnupay_kcp_get_settings('pay_ids');

        return $pay_ids['bank'];   //계좌이체
    }

    public function get_the_title(){
        return __('KCP 계좌이체', GNUPAY_NAME);
    }
    ...
    public function get_the_description() {
        ...
        $return_html = __( 'KCP 계좌이체입니다.', GNUPAY_NAME );
    ...
    public function process_admin_options(){
        ...
        if( $result ){
            ...
            if(isset($kcp_options['de_bank_use']) && $op_enabled != $kcp_options['de_bank_use']){

                $kcp_options['de_bank_use'] = $op_enabled;
